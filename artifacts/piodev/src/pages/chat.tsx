...
export default function ChatPage() {
  ...
  const showPremiumBanner = !isAdmin && !isPremium && !isBannerDismissed && !premiumStatus?.application;
  ...
  return (
    <div className="flex h-dvh w-full bg-background overflow-hidden font-sans">
      ...
      <div className="flex-1 flex flex-col min-w-0 relative">
          ...
          {showPremiumBanner && (
            ...
            >
              <div className="max-w-3xl mx-auto px-4 py-2.5 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <span className="text-xs font-semibold text-amber-600 dark:text-amber-400 mr-1.5">Plus Terbatas — Gratis!</span>
                  <span className="text-xs text-muted-foreground hidden sm:inline">Follow IG kami → 300K token/hari (5× lebih banyak)</span>
                </div>
                <button
                  onClick={() => setLocation("/premium")}
        ...
        {/* Notif pending application */}
        {!isAdmin && !isPremium && premiumStatus?.application?.status === "pending" && (
          <div className="shrink-0 border-b border-border bg-muted/40 px-4 py-2">
            <p className="text-xs text-muted-foreground text-center max-w-3xl mx-auto">
              ...
              Aplikasi Plus Terbatas kamu sedang direview. Ditunggu ya!
            </p>
          </div>
        )}
