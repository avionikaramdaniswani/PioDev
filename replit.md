...
- **Pio Studio (sidebar collapsible)** — section "Pio Studio" di sidebar punya 4 sub-item collapsible (chevron accordion, state disimpan di `localStorage.piodev_studio_expanded`, auto-expand kalau lagi di sub-route): **Galeri Studio** (`/galeri-studio`, coming soon — galeri terpadu hasil semua studio), **Image Studio** (`/image-studio`, coming soon — Qwen-Image dengan kontrol penuh), **Voice Studio** (`/voice-studio`, NEW), **Video Studio** (`/video-studio`, existing). Di mode collapsed, icon Pio Studio (Sparkles) buka dropdown menu untuk pilih sub-studio.
- **Voice Studio** (`/voice-studio`, NEW) — TTS, voice cloning, voice design pake Qwen3-TTS family (qwen3-tts-flash, qwen3-tts-instruct-flash, cosyvoice-v3-flash/plus). 4 tab: **TTS Playground** (textarea + voice picker preset/custom + model + bahasa + instruksi gaya untuk model instruct, generate audio MP3 stream-able + download), **Voice Cloning** (upload sample audio 10-30 detik → DashScope `voice-enrollment` → simpan `voice_id` di tabel `user_voices`), **Voice Design** (prompt deskripsi suara → DashScope `qwen-voice-design` → simpan `voice_id`), **Suaraku** (list custom voices, "Pakai" → load ke TTS playground, hapus). **Kredit voice bulanan tier-aware**: Free 10/bln, Plus 60/bln, Pro 200/bln (admin unlimited). Cost: TTS=1, clone=5, design=10. Kolom `profiles.voice_credits` (TERPAKAI bulan ini, BUKAN sisa) + `voice_credits_reset_date DATE`. Tabel `user_voices (id, user_id, name, type clone|design, dashscope_voice_id, source_text, language, created_at)` dengan RLS. Endpoint: `GET /api/voice-studio/quota`, `GET /api/voice-studio/voices` (presets + custom), `DELETE /api/voice-studio/voices/:id`, `POST /api/voice-studio/tts` (preset, return audio binary), `POST /api/voice-studio/tts-custom` (custom voice via voice_db_id), `POST /api/voice-studio/clone` (multipart `audio` + `name`, max 10MB via `voiceCloneUpload`), `POST /api/voice-studio/design` (JSON prompt+name). Helper: `getVoiceCredits/deductVoiceCredits/callDashscopeTTS` di server. Konstanta: `FREE/PLUS/PRO_VOICE_CREDITS`, `VOICE_COST_TTS/CLONE/DESIGN`, `VOICE_STUDIO_PRESETS` (Cherry/Ethan/Chelsie/Serena/Dylan/Jada). Audio output TIDAK disimpan ke DB (Galeri Studio masih coming soon) — user tinggal download/share langsung. Voice mode chat tetap pake Azure Speech (terpisah dari Voice Studio). **Migrasi**: `server/voice-studio-migration.sql` — WAJIB jalanin manual sekali via Supabase Dashboard SQL Editor.
...
- **API Keys (BYOK) — Plus & Pro** — Plus, Pro & Admin user generate `pio-sk-...` untuk akses PioCode API dari luar. Key disimpan ter-enkripsi (AES-256-GCM via `API_KEY_ENCRYPTION_SECRET`) → bisa di-reveal & copy ulang kayak Gemini AI Studio. Free user GAK bisa bikin (403 + CTA upgrade). **Tier gating per model**: konstanta `PRO_ONLY_MODELS` (server/index.ts) berisi model frontier (`qwen3-max*`, `qwen3-235b-a22b-thinking-2507`, `qwen3-coder-plus*`, `qwen-image-max`, `qwen-image-2.0-pro`, `qwen-image-edit-plus`, `wan2.6-t2v`, `wan2.6-i2v`, `wan2.2-i2v-plus`). Helper `assertProAccess(res, model, tier, isAdmin)` dipanggil di `/v1/chat/completions`, `/v1/images/generations`, `/v1/videos/generations` — user Plus yang minta model Pro-only kena 403 dengan header `X-Pioo-Error: MODEL_PRO_ONLY` + body `{ required_tier: "pro", current_tier: "plus" }`. `requireApiKey` middleware sekarang attach `(req as any).apiTier` (admin diperlakukan sebagai "pro"). Dokumentasi ditampilkan di tab "Models" pada `/api-keys` lewat badge `Pro only` (crown icon, amber) vs `Plus & Pro` (slate) per row.
...
| Voice Studio kredit / bulan | 10 | 60 | 200 | Unlimited |
